...
namespace ARLocation.MapboxRoutes
{
    public class MapboxRoute : MonoBehaviour
    {
        ...
        /// 
        /// </summary>
        public GameObject dog, dog2;
        Animator pet_ani;
        bool dog_spawn, isMoving = false;
        float time, distance_update_timer;
        bool total_distance_set;
        Vector3 prePosition;


        float tmp_x, tmp_y, tmp_z;
        public GameObject test_cube;
        public Text text_tmp;
        ...
        void Start()
        {
            dog_spawn = false;
            time = 0;
            distance_update_timer = 0;
            total_distance_set = false;
            text_tmp.text = PlayerPrefs.GetString("targetaddress");
            //text_tmp.text = "Vater-Rhein-Brunnen, Museumsinsel, Muenchen, Bavaria 80538, Germany";
        ...
        void Update()
        {
            ...
            //if (dog_spawn)
            //   set_dog_position(tmp_x, tmp_y, tmp_z);
        ...
        void set_dog_position(float x, float y, float z)
        {
            // dog2.transform.position = new Vector3(_pos.x, s.StepsPlaceAtInstances[index].transform.position.y, _pos.z * 2);
            test_cube.transform.position = new Vector3(x, y, z);
        ...
        private RoutePathRendererArgs createRoutePathRendererArgs()
        {
            ...
            if (!dog_spawn)
            {
                dog = Instantiate(dog, s.StepsPlaceAtInstances[index + 1].transform.position, Quaternion.identity);
                //dog2 = Instantiate(dog, s.StepsPlaceAtInstances[index + 1].transform.position, Quaternion.identity);
                pet_ani = dog.GetComponent<Animator>();
                dog.transform.localScale = new Vector3(0.7f, 0.7f, 0.7f);
                dog_spawn = true;
                //test_cube = Instantiate(test_cube, s.StepsPlaceAtInstances[index + 1].transform.position, Quaternion.identity);
            }

            tmp_y = s.StepsPlaceAtInstances[index].transform.position.y;


            ///working
            Vector3 _pos = (s.StepsPlaceAtInstances[s.CurrentTargetIndex].transform.position - user).normalized;

            _pos = user + new Vector3(_pos.x * 3f, s.StepsPlaceAtInstances[s.CurrentTargetIndex].transform.position.y, _pos.z * 3f);
            dog.transform.position = _pos;
            dog.transform.LookAt(s.StepsPlaceAtInstances[s.CurrentTargetIndex].transform.position);
            ...
            if (time > 1)
            {
                ...
                if (Vector3.Distance(user, prePosition) > 0.5f)
                {
                    isMoving = true;
                    pet_ani.SetBool("Walk", true);
                }
                else
                {
                    pet_ani.SetBool("Walk", false);
        ...
        private SignPostEventArgs createSignPostEventArgs(int index)
        {
            ...
            //working
            tmp_x = s.StepsPlaceAtInstances[s.CurrentTargetIndex].transform.position.x;
            tmp_z = s.StepsPlaceAtInstances[s.CurrentTargetIndex].transform.position.z;
            //


            Debug.Log(index + " v3 is " + s.StepsPlaceAtInstances[index].transform.position + s.CurrentTargetIndex);

            //Debug.Log(s.RouteSteps[index].maneuver.instruction + " - - - " + MathUtils.HorizontalDistance(user, target));
            if (index == s.StepsPlaceAtInstances.Count - 1)
            {
                //text_tmp.text = MathUtils.HorizontalDistance(user, target)+"";
                total_distance = MathUtils.HorizontalDistance(user, target);
                if (!total_distance_set)
                {
                    PlayerPrefs.SetFloat("total_distance", total_distance);
                    total_distance_set = true;
                }
                else
                {
                    distance_update_timer += Time.deltaTime;
                    if (distance_update_timer > 1)
                    {
                        distance_update_timer = 0;

                        PlayerPrefs.SetFloat("rest_distance", total_distance);
                        text_tmp.text = PlayerPrefs.GetFloat("rest_distance") + "";
                        PlayerPrefs.SetFloat("direction_x", dog.transform.position.x);
                        PlayerPrefs.SetFloat("direction_y", dog.transform.position.z);
                    }
                }
            }
